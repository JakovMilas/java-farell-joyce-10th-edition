...
public class MenuSearch
{
	public static void main(String[] args)
	{
		...
		while(!menuChoices[x].equals("zzz") && x < highestSub)
		{
			...
			if(x < highestSub)
				menuChoices[x] = JOptionPane.showInputDialog(null,
						"Enter an item fr today's menu or zzz to quit");
		}
		numEntered = x;
		entry = JOptionPane.showInputDialog(null, "Today's meny is:\n" + menuString +
		...
		if(x >= 0 && x < numEntered)
			...
			JOptionPane.showMessageDialog(null,"Sorry - that item is not on tonigh's menu");
	}
}
